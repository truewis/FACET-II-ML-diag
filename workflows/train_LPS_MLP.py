...
for _ in range(1):
    if answer == 'y': 
        ...
        np.save('data/processed/LPSimages' + experiment + '_' + runname + '.npy', xtcavImages[:,:,all_idx[goodShots]])
    elif answer == 'n': 
...
def custom_loss( y_pred,y_true): 
    mse = (y_true - y_pred)**2
    weights = 1 + 0.5*((y_true > 200)).float()
...
while True: 
    ...
    if answer == 'y': 
        ...
        import joblib
        import pickle
        joblib_file = 'model/LPS/MLP_LPS_'+experiment+'_'+runname+'.pkl'  
        joblib.dump(model, joblib_file)
        joblib.dump(iz_scaler, 'model/LPS/' + experiment +'_'+runname+'_scaler.gz')
        pickle.dump(pca, open('model/LPS/' + experiment + '_' + runname + '_pca.pkl',"wb"))
        break
    elif answer == 'n': 
        break
    else: 
...
if answer == 'y': 
    ...
    import joblib
    joblib_file = 'model/predictions/predLPS_'+experiment+'_'+runname+'.pkl'  
    x = np.vstack((bsaScalarData, steps)).T
    x=torch.tensor(x_scaler.transform(x), dtype=torch.float32)
    with torch.no_grad():
        pred = pca.inverse_transform(model(x).numpy()).T.reshape(2*yrange,2*xrange,x.shape[0])
